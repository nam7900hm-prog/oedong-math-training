...
export default function TeacherProblemStudioPro({curriculum}:{curriculum:Curriculum}){
  ...
  return <section className="problemStudioPro">
    ...
    {mode==="register"&&<div className="registerLayout">
      <section className="capturePanel"><div className="panelTitle"><b>캡처 문제 바로 등록</b><span>Shift+Win+S → Ctrl+V</span></div><div ref={pasteZone} tabIndex={0} className={`pasteZone ${capture?"hasImage":""}`} onPaste={onPaste} onClick={()=>pasteZone.current?.focus()}>{capture?<img src={capture} alt="붙여 넣은 문제 캡처"/>:<><strong>여기를 클릭하고 Ctrl+V</strong><p>문제·수식·도형·그래프를 한 장으로 붙여 넣습니다.</p><button type="button" onClick={e=>{e.stopPropagation();imageInput.current?.click()}}>이미지 파일 선택</button></>}<input ref={imageInput} hidden type="file" accept="image/*" onChange={e=>e.target.files?.[0]&&readImage(e.target.files[0])}/></div><div className="captureFields"><label>문제 번호<input value={number} onChange={e=>setNumber(e.target.value)} placeholder="예: 12"/></label><label>정답<input value={answer} onChange={e=>setAnswer(e.target.value)} placeholder="예: ③, x=4"/></label><label className="wide">정답·해설<textarea value={explanation} onChange={e=>setExplanation(e.target.value)} placeholder="해설을 직접 입력하거나 해설 캡처 내용을 기록하세요."/></label></div><div className="captureActions"><button className="secondary" onClick={()=>{setCapture("");setNotice("캡처를 비웠습니다.")}}>다시 캡처</button><button onClick={addCapture}>문제보관함에 저장 →</button></div></section>
      <section className="pdfPanel"><div className="panelTitle"><b>PDF 대량 작업 대기열</b><span>여러 파일 동시 선택</span></div><input ref={pdfInput} hidden type="file" accept="application/pdf" multiple onChange={e=>onPdfFiles(e.target.files)}/><button className="pdfDrop" onClick={()=>pdfInput.current?.click()}><strong>＋ PDF 문제지·해설지 여러 개 선택</strong><span>문제지와 정답·해설지를 같은 묶음으로 올릴 수 있습니다.</span></button>{pdfs.length?<div className="pdfQueue">{pdfs.map(pdf=><article key={pdf.id} className={activePdf===pdf.id?"active":""} onClick={()=>setActivePdf(pdf.id)}><input type="checkbox" checked={pdf.checked} onChange={e=>setPdfs(items=>items.map(x=>x.id===pdf.id?{...x,checked:e.target.checked}:x))}/><div><b>{pdf.name}</b><small>{(pdf.size/1024/1024).toFixed(1)}MB · {pdf.status}</small></div><label>예상 문항<input type="number" min="1" max="100" value={pdf.problemCount} onChange={e=>setPdfs(items=>items.map(x=>x.id===pdf.id?{...x,problemCount:Number(e.target.value)||1}:x))}/></label><button onClick={e=>{e.stopPropagation();createPdfSlots(pdf)}}>문항칸 만들기</button></article>)}</div>:<div className="emptyQueue">등록한 PDF가 없습니다.</div>}{activePdf&&pdfs.find(x=>x.id===activePdf)&&<div className="pdfPreview"><iframe title="선택한 PDF 미리보기" src={pdfs.find(x=>x.id===activePdf)!.url}/><p>PDF를 보면서 자동 생성한 문항칸에 캡처·정답·해설을 연결하세요.</p></div>}</section>
    </div>}

    {mode==="bank"&&<section className="bankPanel"><div className="bankToolbar"><div><b>1차 문제보관함</b><span>조각을 만들기 전 원문·정답·해설을 검수합니다.</span></div><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="문제·파일·단원 검색"/><button onClick={()=>persist(problems.map(x=>({...x,checked:true})))}>전체 선택</button></div>{filtered.length?<div className="bankGrid">{filtered.map(item=><article key={item.id} className="bankCard"><div className="bankCardTop"><label><input type="checkbox" checked={item.checked} onChange={e=>updateProblem(item.id,{checked:e.target.checked})}/><b>{item.title}</b></label><span className={item.status.replaceAll(" ","")}>{item.status}</span></div><div className="bankSource">{item.image?<img src={item.image} alt={`${item.title} 원문`}/>:<div className="pdfPlaceholder"><b>PDF 원문 연결 대기</b><small>{item.source}</small><button onClick={()=>{setActiveId(item.id);setMode("register")}}>캡처 붙이기</button></div>}</div><div className="bankAnswers"><label>정답<input value={item.answer} onChange={e=>updateProblem(item.id,{answer:e.target.value,status:e.target.value?"보관 완료":"검수 필요"})}/></label><label>해설<textarea value={item.explanation} onChange={e=>updateProblem(item.id,{explanation:e.target.value})} placeholder="풀이 과정·교사 메모"/></label></div>{renderSelectors(item)}<div className="bankActions"><button className="danger" onClick={()=>{if(confirm("이 문제를 보관함에서 삭제할까요?"))persist(problems.filter(x=>x.id!==item.id))}}>삭제</button><button onClick={()=>openPieces(item)}>큰 화면에서 조각 만들기 →</button></div></article>)}</div>:<div className="bankEmpty"><b>아직 보관된 문제가 없습니다.</b><p>캡처 이미지를 붙여 넣거나 PDF 문항칸을 먼저 만들어 주세요.</p><button onClick={()=>setMode("register")}>문제 등록으로 이동</button></div>}</section>}
