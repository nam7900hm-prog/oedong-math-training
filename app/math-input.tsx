...
type Props={value:string;onChange:(value:string)=>void;label?:string;compact?:boolean;unit?:string;coachText?:string;coachTranslation?:string;purpose?:string;preferredTab?:string};

export function MathInput({value,onChange,label="답을 입력하세요",unit,coachText,coachTranslation,purpose,preferredTab}:Props){
  ...
  return <div className={`mathInput ${keyboard.isActive(id)?"active":""}`}>
    ...
    <div className="mathCoach"><button type="button" onClick={()=>setCoachOpen(current=>!current)} aria-expanded={coachOpen}><span>💡</span><b>풀이 실마리</b><small>{coachOpen?"접기":"막막하면 보기 · 감점 없음"}</small></button>{coachOpen&&<div><b>선생님의 풀이 실마리</b><p>{coach}</p>{coachTranslation&&<p className="coachTranslation">{coachTranslation}</p>}</div>}</div>
  </div>;
}
